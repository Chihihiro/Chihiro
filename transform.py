...
class CleanWrong(BaseTransform):
    ...
    def process(self, frame):
        ...
        for key, patt in self._patts.items():
            ...
            # todo this method will always replace blank to None
            frame.loc[frame[key].apply(lambda x: cpatt.sub(flag, x) if type(x) is str else flag) == flag, key] = self._repls.get(key)
        return frame
...
class Join(BaseTransform):
    ...
    def process(self, frame):
        return frame.merge(self._right.dataframe, how=self._how, on=self._on, left_on=self._left_on, right_on=self._right_on,
