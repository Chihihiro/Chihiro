...
def init_2():
    ...
    """
    sql_com = "SELECT fund_name, fund_id, foundation_date from fund_info"
    engine = create_engine('mysql+pymysql://jr_read_17:jr_read_17@182.254.128.241:4171/base?charset=utf8')
    df = pd.read_sql_query(sql_com, engine)
    nrow = len(df)
    print(nrow)
...
def init_2_f():
    ...
    """
    sql_com = "SELECT fund_full_name, fund_id, foundation_date from fund_info"
    engine = create_engine('mysql+pymysql://jr_read_17:jr_read_17@182.254.128.241:4171/base?charset=utf8')
    df = pd.read_sql_query(sql_com, engine)
    nrow = len(df)
    print(nrow)
...
def init_2_f_aft(fund_name2, nrow, colname):
    """
...
def found_time_match(fund_id1,fund_id2,df_data):
    if df_data['foundation_date'][df_data.fund_id==fund_id1].empty or df_data['foundation_date'][df_data.fund_id==fund_id2].empty:
        return True
    if df_data['foundation_date'][df_data.fund_id==fund_id1].tolist()[0]==df_data['foundation_date'][df_data.fund_id==fund_id2].tolist()[0]:
        return True
    else:
        return False
...
def raw_list(fund_name, fund_name2_aft, col_name, range, primarykey):
    ...
    fund_name2_fundid = fund_name2_aft['fund_id'].tolist()
    if primarykey:
    ...
    for j in range:
        ...
        if len(match1) != 0 and len(match2) != 0 and fund_name[1]!=fund_name2_fundid[j] and match1.issubset(match2) and found_time_match(fund_name2_fundid[j],fund_name[1],fund_name2):  # 快速准则
            t2 = time.time()
            if t2 - t1 > 30:
                return np.nan
            if primarykey:
...
def choose_best(fund_found_list, model):
    ...
    for i in range(nlist):
        ...
        if temp != 0:
            ...
            temp = 10000
        if temp <= min_dis:
            temp2 = ld.distance(sub_chi2num(col_chi(strQ2B(fund_name_t))),
                                sub_chi2num(col_chi(strQ2B(fund_found_list[i][1]))))
            if temp2 < min_ld:
                min_dis = temp
                min_ld = temp2
                min_index = i
...
def match_name(fund_name, fund_name2, range, range2, col_name, col_name2, primary_key=None, how=1):
    ...
    # 格式化比对名称
    fund_name2_aft = init_2_f_aft(fund_name2, nrow, col_name2)
    if is_primary_key:
...
def nav_match(fund_id1,fund_id2,engine=create_engine('mysql+pymysql://jr_read_17:jr_read_17@182.254.128.241:4171/base?charset=utf8')):
    """
    ...
    sql_com="SELECT b.statistic_date, a.statistic_date, a.fund_id,b.fund_id,b.nav ,a.nav FROM  \
    (SELECT  fund_id,  fund_name,  statistic_date,  \
    nav FROM  fund_nv_data_standard WHERE fund_id ='{}' ORDER BY statistic_date DESC) as a , \
    (SELECT  fund_id,  fund_name,  statistic_date,    \
    nav FROM crawl_private.d_fund_nv  WHERE  fund_id ='{}' and source_id='020001' ORDER BY statistic_date DESC) as b  \
    where a.statistic_date=b.statistic_date limit 5".format(fund_id1,fund_id2)
...
def row_nav_match(mat_ser,engine=create_engine('mysql+pymysql://jr_read_17:jr_read_17@182.254.128.241:4171/base?charset=utf8')):
    """
    辅助函数，便于apply调用
    从展开后的匹配时间序列中，获取fund_id并进行净值匹配

    Args:
    mat_ser - Series , engine - Create_engine.engine

    Returns:
    is_match - bool

    """
    fund_id1 = mat_ser.tolist()[1]
    fund_id2 = mat_ser.tolist()[3]
    is_match=nav_match(fund_id1,fund_id2,engine)
    return is_match
...
if __name__ == "__main__":
    ...
    #[fund_name,fund_name2,ntest,nrow]=init_update()  # 从数据库更新数据时使用
    # [fund_name, fund_name2, ntest, nrow] = init_local()  # 直接读取本地文件时使用
    init_jieba()
    ...
    res_data=res_data.dropna(how='any')
    new_tes = reshape(res_data,'fund_id',1)
    new_tes['is_nav_match'] = new_tes.apply(row_nav_match,axis=1)
    # new_tes=new_tes[new_tes.is_nav_match==1]   #不用净值匹配
    writer = pd.ExcelWriter('cf-20000_30000.xlsx')
    new_tes.to_excel(writer, "Sheet1")
    writer.save()
