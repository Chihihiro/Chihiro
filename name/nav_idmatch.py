...
def _init_fund_info():
    ...
    d_fund_info=pd.read_sql_query("select DISTINCT fund_id,source_id ,fund_name,fund_full_name from d_fund_info",engine)
    print('d_fund_info')
...
def _nav_match(fund_id1,fund_id2,source_1,engine=create_engine('mysql+pymysql://jr_read_17:jr_read_17@182.254.128.241:4171/crawl_private?charset=utf8')):
    """
...
def nav_idmatch(input,start,end,is_load=False):
    ...
    candidate=input
    tyc_word=['号', '计划', '资产', '基金', '投资', '管理', '私募', '合伙', '信托', '期', '集合', '证券', '专项', '有限', '号', '期', '类', '级', '份额']
    _init_jieba()
    if is_load:
