...
def show_nv(fund_id):
    ...
    if '000001' in column:
        ...
        for i in source:
            DA3.insert(DA3.columns.tolist().index(i)+2, i+'分红', value=None)  # 在 B 列前面插入
            DA3.insert(DA3.columns.tolist().index(i)+3, i+'拆分', value=None)  # 在 B 列前面插入
            DA3[i+'分红'] = list(map(lambda x, y: y-x, DA3[i].iloc[:, 0], DA3[i].iloc[:, 1]))
            DA3[i+'拆分'] = (DA3[i+'分红'] - DA3[i+'分红'].shift(1)) / DA3[i+'分红'].shift(1)
        return DA3
