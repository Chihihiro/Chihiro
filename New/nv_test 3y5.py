...
def is_B8(JR, statistic_date):
    return engine_data_test.execute(
        "update fund_nv_data_standard_test set is_abnormal=8 where fund_id='{}' and statistic_date ='{}' ".format(JR,
                                                                                                                  statistic_date))
def is_B9(JR, statistic_date):
    return engine_data_test.execute(
        "update fund_nv_data_standard_3000 set is_abnormal=9 where fund_id='{}' and statistic_date='{}' ".format(JR,
                                                                                                                 statistic_date))
...
def yansuan(JR):
    nv = pd.read_sql(
        "select fund_id,statistic_date,nav,added_nav from fund_nv_data_standard WHERE fund_id='{}'".format(JR),
        engine_base)
    # if df.empty:
    #     pass
    # else:
    ...
    if nv.empty:
        ...
        try:
            ...
            to_sql("fund_nv_data_standard_3000", engine_data_test, nv,
                   type="update")  # ----------------------------------------输入1的异常
            if df_3.empty:
                print(JR)
                print("No problem")
...
dfjr = pd.read_sql("SELECT PK FROM config_private.`sync_source` WHERE source_id LIKE '03____'", engine_config_private)
dfjr2 = dfjr["PK"]
jr_list = to_list(dfjr2)
for JR in jr_list:
    yansuan(JR)
check()
print("DOWN" + '\n' + now2)
