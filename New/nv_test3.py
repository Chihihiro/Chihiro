...
def is_B8(JR, statistic_date):
    return engine_data_test.execute(
        "INSERT INTO fund_nv_data_standard_test (fund_id,statistic_date, is_abnormal)  VALUES ('{}' ,'{}' ,8)".format(
            JR, statistic_date))


def is_B9(JR, statistic_date):
    return engine_data_test.execute(
        "INSERT INTO fund_nv_data_standard_test (fund_id,statistic_date, is_abnormal)  VALUES ('{}' ,'{}' ,9)".format(
            JR, statistic_date))
...
def crawl(list):
    for JR in list:
        nv = pd.read_sql(
            "select fund_id,fund_name,statistic_date,nav,added_nav FROM fund_nv_data_standard where fund_id = '{}'".format(
                JR), engine_base)
        # nav=nv["nav"]
