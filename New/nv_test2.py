...
def crawl(list):
    ...
    for JR in list:
        ...
        if (nv["nav"] == nv["added_nav"]).all():
            ...
            to_sql("fund_nv_data_standard", engine5, df,
                   type="update")  # ----------------------------------------输入1的异常
            print("异常")
            print(JR)
...
def check():
    df = pd.read_sql(
        "SELECT fund_id,statistic_date,nav FROM fund_nv_data_standard where is_abnormal in (1,2,3,4,5,8,9)  and remark is NULL ",
        engine5)
    # df = pd.read_sql(
