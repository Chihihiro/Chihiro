...
def is_B0(is_ab, JR, statistic_date):
    return engine_data_test.execute("update fund_nv_data_standard_3000 set is_abnormal= {} where fund_id='{}' and statistic_date\
                                                    ='{}' ".format(is_ab, JR, statistic_date))


# def is_B8(JR, statistic_date):
#     return engine_data_test.execute(
#         "INSERT INTO fund_nv_data_standard_3000 (fund_id,statistic_date, is_abnormal)  VALUES ('{}' ,'{}' ,8)".format(
#             JR, statistic_date))


# def is_B9(JR, statistic_date):
#     return engine_data_test.execute(
#         "INSERT INTO fund_nv_data_standard_3000 (fund_id,statistic_date, is_abnormal)  VALUES ('{}' ,'{}' ,9)".format(
#             JR, statistic_date))
...
def crawl(list):
    for JR in list:
        nv = pd.read_sql(
            "select fund_id,statistic_date,nav,added_nav FROM fund_nv_data_standard_copy2 where fund_id = '{}'".format(
                JR), engine_base)
        if (nv["nav"] == nv["added_nav"]).all():
            if len(nv) <= 2:
                ...
                try:
                    diff = abs(
                        nv[["statistic_date", "nav", "added_nav"]] - nv[["statistic_date", "nav", "added_nav"]].shift(1))
                    diff = diff[1:]
                    ...
                    if dif.empty:
                        ...
                        # to_sql("fund_nv_data_standard_test", engine_data_test, df,
                        #        type="update")  # ----------------------------------------输入1的异常
                        print("异常")
                        print(JR)
                        ...
                        for i in range(num):
                            ...
                            if day <= 3 and bo > 0.3:
                                ...
                                print("小小波")
                                pass
                            elif 5 >= day > 3 and bo > 0.4:
                                is_B0(5, JR, stat)
                                print("小波")
                                pass

                            elif 14 >= day >= 6 and bo > 0.4:
                                is_B0(5, JR, stat)
                                print("中波动")
    ...
    for i in x:
        ...
        try:
            df = pd.read_sql("SELECT fund_id,data_source,data_source_name,nav,added_nav\
                            from fund_nv_data_source where fund_id='{}' and statistic_date='{}'".format(JR, time),
                             engine_base)
...
def len_JR():
    JR_all = pd.read_sql("select DISTINCT fund_id FROM fund_nv_data_standard_copy2", engine_base)
