...
def _get_all_nav(fund_id):
    ...
    nav_list=[]
    for ii in candidate:
        sql_nav="select nav,statistic_date from fund_nv_data_source_copy2 where fund_id='{0}' and source_id='{1}'".format(fund_id,ii)
...
def _get_all_addednav(fund_id):
    ...
    nav_list=[]
    for ii in candidate:
        sql_nav="select added_nav,statistic_date from fund_nv_data_source_copy2 where fund_id='{0}' and source_id='{1}'".format(fund_id,ii)
...
def _evaluate(source_id,source,fund_id,foundation_date,all_data):
    ...
    quality={'源':source,'源id':source_id,'不重复率':_cal_quality(all_data.loc[:,[source]]),'覆盖率':_complete_rate(all_data.loc[:,[source]],foundation_date),'众数率':_rate_mode(all_data,source)}
    return quality
...
def _get_id_foundation():
    ...
    for ii in candidate:
        print(ii)
        sql_com_get_match = "select distinct matched_id,source_id from id_match where source='{}' and is_used=1 and matched_id like 'JR%%'".format(ii)
...
def fund_id_best(fund_id):
    global gg
    engine = create_engine('mysql+pymysql://jr_read_17:jr_read_17@182.254.128.241:4171/base?charset=utf8')
    sql_com="select matched_id,source_id,source,foundation_date from id_match  left join fund_info on id_match.matched_id=fund_info.fund_id where  is_used=1 and matched_id = '{}'".format(fund_id)
    ...
    for i in range(len(candidate_source)):
        try:
            temp=_evaluate(candidate_source.loc[i,'source_id'],candidate_source.loc[i,'source'],fund_id,candidate_source,all_data)
        except:
    ...
    for i in range(len(candidate_source)):
        try:
            temp=_evaluate(candidate_source.loc[i,'source_id'],candidate_source.loc[i,'source'],fund_id,candidate_source,all_data_add)
        except:
            return "没有净值数据","没有净值数据","没有净值数据","没有净值数据","没有净值数据"
...
where sy.pk is NULL AND i.matched_id is not NULL 	"
    ...
    bad_list=[]
    for id in wanted_list[:1000]: #这里改
        ii=fund_id_best(id)
        if not isinstance(ii[1],str):
            for jj in ii[1]:
                if (not np.isnan(jj['众数率'])) and jj['众数率']<0.5:
                    bad_list.append(['净值',id,jj['源id']],jj['众数率'])
            for jj in ii[1]:
                if (not np.isnan(jj['众数率'])) and jj['众数率']<0.5:
                    bad_list.append(['累计净值',id,jj['源id']],jj['众数率'])
    print(bad_list)